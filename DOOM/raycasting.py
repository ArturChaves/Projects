...
class RayCasting:
    def __init__(self, game):
        ...
        self.textures = self.game.ObjectRender.wall_textures
    
    def get_objects_to_render(self):
        self.objects_to_render = []
        for ray, values in enumerate(self.ray_casting_result):
            depth, projection_height, texture, offeset = values
            if projection_height < height:
                wall_column = self.textures[texture].subsurface(
                    offeset * (texture_size - scale), 0, scale, texture_size
                )
                ...
                wall_column = self.textures[texture].subsurface(
                    offeset * (texture_size - scale), half_texture_size - texture_height // 2,
